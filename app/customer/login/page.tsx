...
function LoginContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect happens via the useEffect above once customer state updates
    } catch (err: any) {
  ...
  // The form is ALWAYS visible — never block with a full-page spinner.
  // While auth is initializing (checking existing session), form is disabled
  // with a subtle "Checking session..." hint so user knows why they can't type yet.
  const formDisabled = authLoading || isSubmitting;
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8">
            ...
            {/* Session check hint — visible only during auth init */}
            {authLoading && (
              <div className="mb-5 flex items-center justify-center gap-2 text-xs text-blue-600 bg-blue-50 rounded-lg py-2.5 px-4 border border-blue-100">
                <Loader2 className="h-3.5 w-3.5 animate-spin shrink-0" />
                <span>Checking session, please wait…</span>
              </div>
            )}
