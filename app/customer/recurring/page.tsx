...
import { ArrowLeft, RefreshCw, Plus, Repeat, Trash2, Play, Calendar, AlertCircle, CheckCircle } from 'lucide-react';
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push('/customer/dashboard')} className="text-gray-500 hover:text-gray-900 transition-colors">
            ...
            <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                <div>
                  ...
                  <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Monthly AC maintenance"
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input type="datetime-local" value={endDate} onChange={e => setEndDate(e.target.value)} min={startDate || minDate}
                      className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-purple-500" />
                  </div>
                </div>
        ...
        {/* List */}
        {isLoading ? (
          <div className="space-y-3">{[1,2].map(i => <div key={i} className="h-24 bg-white rounded-xl border border-gray-200 animate-pulse" />)}</div>
        ) : jobs.length === 0 ? (
            ...
            {jobs.map((job, i) => {
              ...
              return (
                <motion.div key={job.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                  className={`bg-white rounded-xl border p-5 transition-all ${job.is_active ? 'border-gray-200' : 'border-gray-100 opacity-60'}`}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      {job.description && <p className="text-xs text-gray-500 mb-2 line-clamp-1">{job.description}</p>}
                      <div className="flex items-center gap-3 text-xs text-gray-500 flex-wrap">
                        <span className="flex items-center gap-1">
                          <Repeat className="h-3 w-3 text-purple-500" />
                          {PATTERN_LABELS[job.pattern]}
                        </span>
                        <span className="flex items-center gap-1">
                        ...
                        {job.end_date && (
                          <span className="text-gray-400">Ends {format(new Date(job.end_date), 'MMM d, yyyy')}</span>
                        )}
                      </div>
                    </div>
                    {job.is_active && (
                      <div className="flex items-center gap-2 shrink-0">
                        <button onClick={() => handleRunNow(job.id)} disabled={actionLoading === job.id + '_run'}
                          title="Trigger now"
                          className="p-2 rounded-lg text-purple-600 hover:bg-purple-50 transition-all disabled:opacity-50">
                          {actionLoading === job.id + '_run' ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}
                        </button>
                        <button onClick={() => handleCancel(job.id)} disabled={actionLoading === job.id + '_cancel'}
                          title="Cancel recurring job"
                          className="p-2 rounded-lg text-red-500 hover:bg-red-50 transition-all disabled:opacity-50">
                          {actionLoading === job.id + '_cancel' ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                        </button>
                      </div>
                    )}
                  </div>
