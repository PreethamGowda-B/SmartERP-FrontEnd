export default function Head() {
  return (
    <>
      <title>SmartERP – Crew Management System</title>
      <meta
        name="description"
        content="SmartERP helps manage jobs, employees, payroll, and materials efficiently."
      />
      <meta
        name="keywords"
        content="SmartERP, crew management, employee portal, payroll, construction management"
      />

      {/* Google Site Verification */}
      <meta name="google-site-verification" content="GGqZAzDXY-Lx_2WoogT65yEOPFtaR87ZCt0NT7MiV2g" />

      <meta name="viewport" content="width=device-width, initial-scale=1" />
      <link rel="icon" href="/placeholder-logo.svg" />
    </>
  )
}
